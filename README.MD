...
这里的接口简单规划为POST方法，参数名为'txt'，使用JSON返回；
整个API服务端放在/mobile_apisvr/目录下
...
![运行界面截图](https://github.com/xmxoxo/BERT-train2deploy/blob/master/images/cap01.png?raw=true)

可以看到请求的用时时间为37ms


附件：
本例中训练完成的模型文件.ckpt格式及.pb格式文件，由于比较大，要放到网盘稍后提供下载。
